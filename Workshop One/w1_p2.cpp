...
parameter T175,SComputer Starting,P, codes T - time (parameter: a number
representing the time--measured in seconds--when the following codes apply) S -
start event (parameter: a string representing the description for the event that
starts) E - end the event P - print to screen A - archive
*/
    ...
    for (auto day = 1; day < argc; ++day) {
        ...
        for (::g_sysClock = 0u; ::g_sysClock < secInDay; ++::g_sysClock) {
            ...
            while (time == ::g_sysClock) {
                ...
                switch (opcode) {
                case 'T': // a new time code, this is the exit from the while loop (back
                          // to for loop)
                    in >> time;
                    break;
